...
import { Trash2, Edit, ChevronLeft, Star, ShieldCheck, Heart, Share2, MapPin, MessageCircle, BadgeCheck, HandCoins, X, ShoppingCart } from 'lucide-react';
...
const ProductDetails: React.FC = () => {
  ...
  const submitOffer = (e: React.FormEvent) => {
    ...
    const conversationId = createConversation(product.seller, product);
    const text = `Offer: I would like to buy this for ${Number(offerPrice).toLocaleString()} ${product.currency}. ${offerMessage}`;
    ...
    addNotification({
      type: 'success',
      title: 'Offer Sent',
      message: `Your offer of ${Number(offerPrice).toLocaleString()} ${product.currency} has been sent to ${product.seller.name}.`
    });
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            ...
            >
              <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-teal-50">
                <h3 className="font-bold text-teal-900">Make an Offer</h3>
              ...
              <form onSubmit={submitOffer} className="p-6">
                <div className="flex items-center gap-3 mb-6 bg-gray-50 p-3 rounded-lg">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-bold text-gray-900">Listed: {product.price.toLocaleString()} {product.currency}</p>
                  </div>
                </div>

                <div className="mb-4">
                  <label className="block text-sm font-bold text-gray-700 mb-2">Your Offer ({product.currency})</label>
                  <input
                    type="number"
                    value={offerPrice}
                    ...
                    required
                  />
                  <p className="text-xs text-gray-400 mt-1">Suggested: {(product.price * 0.9).toLocaleString()} - {product.price.toLocaleString()}</p>
                </div>
