...
const Messages: React.FC = () => {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const response = await fetch('http://localhost:8000/api/upload/index.php', {
        method: 'POST',
        body: formData,
      });
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    setIsSearching(true);
    try {
      const response = await fetch(`http://localhost:8000/api/messages/index.php?action=search&userId=${user?.id}&query=${encodeURIComponent(searchQuery)}`);
      const data = await response.json();
      if (data.success) {
